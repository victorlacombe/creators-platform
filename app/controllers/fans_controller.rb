class FansController < ApplicationController
  ...
  def index
    ...
    if params[:query].present?
      ...
      @fans = current_user.fans.where(sql_query, query: "%#{params[:query]}%").where.not(channel_id_youtube: current_user.channel_id_youtube).page(params[:page]).per(4*3)
    else
      # .page is from Kaminari gem
      @fans = current_user.fans.where.not(channel_id_youtube: current_user.channel_id_youtube).page(params[:page]).per(4*3)
    end
